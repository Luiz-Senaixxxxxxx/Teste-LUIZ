...
async function getData() {
    const response = await fetch("https://api.zerosheets.com/v1/gpo");
...
async function patchRow(lineNumber, payload) {
    const url = "https://api.zerosheets.com/v1/gpo/" + lineNumber;
...
async function deleteRow(lineNumber) {
    const url = "https://api.zerosheets.com/v1/gpo/" + lineNumber; // lineNumber comes from the get request
    await fetch(url, {
        method: "DELETE"
    });
}
