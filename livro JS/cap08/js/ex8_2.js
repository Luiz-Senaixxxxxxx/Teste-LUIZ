...
frm.addEventListener("submit", (e) => {
    e.preventDefault()
    //obter o conteùdo do form
    const modelo = frm.inModelo.value
    const ano = Number(frm.inAno.value)
    const preco = Number(frm.inPreco.value)
    //chamar a function classificarVeiculo e salvar na variável classiicação
    const classificacao = classificarVeiculo(ano)
    //cahamr a function calcularEntrada e salvar na variável entrada
